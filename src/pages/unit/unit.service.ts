...
export class UnitService {
   ...
   * insertManyUnit()
   */
  async addUnit(addUnitDto: AddUnitDto): Promise<ResponsePayload> {
    try {
      const createdAtString = this.utilsService.getDateString(new Date());
      const data = new this.unitModel({ ...addUnitDto, createdAtString });
  ...
  async getUnitById(id: string, select: string): Promise<ResponsePayload> {
    try {
      const data = await this.unitModel.findById(id).select(select);
